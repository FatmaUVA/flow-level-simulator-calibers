...
class Scheduler:
    ...
    #involved flows includes flow id, and involved links includes liks id (not the object itself)
    def find_max_rate_per_flow(self):
        global C
        done_flows = [] #list of flows received the max rate in their bottleneck link

        involved_links = self.topo.links #consider all links
        involved_flows = [] #hold flows IDs
        for f in self.flows:
        ...
        #Sort links by the number of flows spanning the liks
        #create a dicrionay for links
